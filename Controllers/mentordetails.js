...
exports.getmentordetailsByLangId = (req, res) => {
  ...
  MentorDetails.find({ mentor: { language_id: langId } })
    .then((response) => {
      res.status(200).json({
...
exports.filteredmentordetails = (req, res) => {
  ...
  language && (filterObj["mentor"] = {});
  language && (filterObj["mentor"].language = language);
  feature && (filterObj["features_id"] = { $in: feature });
  lcost && hcost && (filterObj["cost"] = { $gte: lcost, $lte: hcost });
  MentorDetails.find(filterObj)
    .sort({ cost: sort })
    .then((response) => {
...
exports.getmentordetailsByMentorId = (req, res) => {
  ...
  MentorDetails.findById(mentorId)
