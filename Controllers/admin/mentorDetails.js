const MentorDetails=require('../../Models/admin/mentorDetails')

exports.getmentordetailsforAdmin=(req,res)=>{
   MentorDetails.find().then(response=>{
        res.status(200).json({
...
exports.deleteMentorByIdforAdmin=(req,res)=>{
    MentorDetails.findByIdAndDelete(req.body.id).then(response=>{
        res.status(204).json({
...
exports.createMentorforAdmin=(req,res)=>{
    MentorDetails.create(req.body).then(response=>{
        res.status(200).json({
